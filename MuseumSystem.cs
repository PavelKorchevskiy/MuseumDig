    ...
    public void LoadFromSaveData(MuseumSaveData data)
    {
        ...
        foreach (var roomData in data.Rooms)
        {
            var room = new Room
            {
                ...
                Width = 10, // Или roomData.Width, если сохраняется
                Height = 10, // Или roomData.Height
                IsMainHall = roomData.Id == "main_hall"
            };

            room.InitializeGrid(); // Этот метод, вероятно, инициализирует массив bool[,]
            ...
            // Восстанавливаем мебель и коллекции
            if (roomData.Furniture != null)
            {
                foreach (var furnData in roomData.Furniture)
                {
                    ...
                    // 2. НОВОЕ: Загрузка собранных коллекций
                    else if (GameData.GetCollection(furnData.FurnitureTypeId) != null)
                    {
                        var collection = GameData.GetCollection(furnData.FurnitureTypeId);
                        furniture = new CollectionExhibit(collection);
                    }
                    // 3. Пьедесталы игнорируются (старые сохранения просто не загрузят этот тип)
                    ...
                    if (furniture != null)
                    {
                        var placed = new PlacedFurniture
                        {
                            ...
                            Size = new Vector2I(furnData.SizeX, furnData.SizeY),
                            IsFlipped = furnData.FurnitureSaveData.IsFlipped,
                            Furniture = furniture
                        };
                        ...
                        if (furniture is DisplayCase && furnData.FurnitureSaveData?.DisplayCaseItems != null)
                        {
                            ...
                            {
                                placed.Items = new List<FoundItem>();
                            }
                            else
                            {
                                placed.Items.Clear();
                            ...
                            // Копируем предметы из сохранения в размещенную витрину
                ...
                foreach (var placed in room.PlacedFurnitureList)
                {
                    ...
                    {
                        for (int y = placed.Position.Y; y < placed.Position.Y + placed.Size.Y; y++)
                        {
                            if (x >= 0 && x < room.Width && y >= 0 && y < room.Height)
                            {
                                room._occupancyGrid[x, y] = true;
                            }
                        }
                    }
                }
